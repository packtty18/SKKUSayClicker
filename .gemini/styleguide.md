...
이 문서는 프로젝트 내 C# 코드의 일관성, 가독성, 유지 보수성 확보를 위한 공식 코딩 규칙을 정의합니다.  
모든 Pull Request(PR)는 아래 명시된 규칙을 준수해야 합니다.

---

## 0. 코드 리뷰 봇 지침 (Gemini Code Assistant)

- **리뷰 언어:** Gemini Code Assistant는 모든 코드 리뷰 요약 및 코멘트를 **한국어(Korean)**로 작성해야 합니다.  
  한국인이 쉽게 이해할 수 있도록 명확하고 자연스러운 한글 설명을 사용하며, 영어나 기타 언어 사용은 허용되지 않습니다.

- **디자인 원칙 검토:** 모든 PR에 대해 **SOLID 원칙**과 **디미터의 법칙(Law of Demeter)** 위반 여부를 중점적으로 확인하고 피드백을 제공합니다.
...
| 약어 | 원칙 | 핵심 요약 |
| :---: | :--- | :--- |
| **S** | 단일 책임 원칙 (SRP) | 클래스는 **단 하나의 변경 이유**만 가져야 하며, 하나의 책임만 담당하도록 설계합니다. |
| **O** | 개방-폐쇄 원칙 (OCP) | 코드는 **확장에는 열려 있어야**, **수정에는 닫혀 있어야** 합니다. 기존 코드 변경 없이 기능을 확장할 수 있어야 합니다. |
| **L** | 리스코프 치환 원칙 (LSP) | 상위 타입 객체를 하위 타입 객체로 **치환해도 프로그램이 문제없이 작동**해야 합니다. |
| **I** | 인터페이스 분리 원칙 (ISP) | 클라이언트가 사용하지 않는 인터페이스에 의존하지 않도록, **단일 목적의 작은 인터페이스**를 선호합니다. |
| **D** | 의존성 역전 원칙 (DIP) | **고수준/저수준 모듈 모두 추상화에 의존**해야 합니다. 구체 구현에 의존하면 안 됩니다. |

#### 0.1.2. 디미터의 법칙 (Law of Demeter, LoD)

- **핵심 지침:** 객체 간 결합도를 낮추고 캡슐화를 강화합니다.  
  **"오직 가장 가까운 친구와만 이야기하라"**는 원칙을 따릅니다.

- **준수 범위:** 메서드 내부에서는 다음 객체들의 메서드만 호출합니다.
  1. 현재 객체 **자신**
  2. 메서드의 **매개변수로 전달된 객체**
  3. 현재 객체의 **필드(멤버 변수)**로 가지고 있는 객체
  4. 메서드 내에서 **새로 생성된 객체**

- **위반 방지:** `a.getB().getC().doSomething()`과 같은 **"기차 참사(Train Wreck)"** 패턴을 피합니다.

---

### 0.2. 보이스카웃 법칙 (Scout Rule)

> **"코드를 항상 내가 발견했을 때보다 더 깨끗한 상태로 남겨라."**

#### 0.2.1. 적용 범위

- **기존 클래스/파일 수정 시에만 적용**합니다.  
  새로 생성하는 클래스나 기능에는 강제하지 않지만, 기존 코드 정리 권장은 가능합니다.

#### 0.2.2. 적용 지침

- 기존 클래스 수정 시, 발견한 기술 부채를 가능한 한 함께 정리합니다.  
  (예: 네이밍 개선, 불필요한 코드 제거, 조건문 단순화 등)

- 새로운 기능 추가 시, 기존 코드의 위험한 부분이나 중복된 구조가 있다면 통합/정리합니다.

- PR 제출 전, 변경 파일 주변 코드까지 포함하여
  **스타일 위반, 불필요한 using, 죽은 코드, 불필요한 SerializedField** 등을 정리합니다.
...
- 긍정적 평가:
  - 기존 파일 구조 개선(SOLID 강화)  
  - LoD 위반 개선  
  - 가독성 향상 리팩터링  
  - 예외 처리 추가

- 경고/변경 요청:
  - 기존 파일을 더 난해하게 만드는 변경  
  - 결합도 증가(LoD 위반)  
  - 기존 구조 악화
...
```csharp
// ❌ 기존 코드
_playerData.healthSystem.currentHealth -= damage;

// ✔ 개선 코드
_playerData.ApplyDamage(damage);

// ✔ PR 설명 예시
// - PlayerData의 체력 접근 로직에서 LoD 위반 수정
// - ApplyDamage 메서드 추가로 캡슐화 강화 및 코드 의도 명확화
